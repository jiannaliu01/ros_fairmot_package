...
def eval_seq_single(opt, img, img0, tracker, results, frame_id, data_type, result_filename, save_dir=None, show_image=True, frame_rate=30):
    if save_dir:
        mkdir_if_missing(save_dir)
    timer = Timer()
    # results = []
    ...
    timer.tic()
    blob = torch.from_numpy(img).cuda().unsqueeze(0)
    print("SHAPES", img.shape, img0.shape, blob.shape)
    online_targets = tracker.update(blob, img0)
    online_tlwhs = []
    online_ids = []
        #online_scores = []
